...
import { getAllCapaigns, deleteCampaigns, createCampaign } from '../api/campaign';
...
class Campaigns extends React.Component<{ campaigns: ICampaign[] }, State> {
	static getInitialProps = async ({ res }) => {
		return {
			campaigns: (res ?
				res.locals.campaigns :
				await getAllCapaigns()
			),
		};
	}
	...
	refreshList = async () => {
		try {
			const campaigns = await getAllCapaigns();
